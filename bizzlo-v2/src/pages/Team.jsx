          ...
          <div className="dashboard-columns">
            <Panel title="Create Partner Manager" description="This creates the partner record and sends it to the secure invite queue.">
              <form className="form-grid compact-form" onSubmit={handlePartnerSubmit}>
                <TextInput label="Partner company" required value={partner.organization_name} onChange={(event) => setPartner({ ...partner, organization_name: event.target.value })} />
                ...
                {partnerOrgs.map((organization) => {
                  ...
                  return (
                    <article className="partner-card" key={organization.id}>
                      ...
                      <span>Manager: {manager?.name || 'Awaiting setup'} · Counselors: {counselors.length}/{organization.counselor_limit || 1}</span>
                    </article>
                  );
                })}
              </div>
